...
import { useDeleteCurriculum } from "../hooks/use-delete-curriculum";
import type { Curriculum } from "../types/curriculum.type";
...
export function CurriculumTable({ curriculums, onEdit }: CurriculumTableProps) {
  const deleteMutation = useDeleteCurriculum();
  const confirm = useConfirm();

  const canUpdate = hasPermission("curriculum:update");
  const canDelete = hasPermission("curriculum:delete");
        ...
        <Table>
          <TableHeader className="bg-slate-50/50">
            <TableRow className="border-b border-slate-100">
              <TableHead className="font-semibold text-slate-600 h-12">Tên giáo trình</TableHead>
              <TableHead className="font-semibold text-slate-600 h-12">Khóa học liên kết</TableHead>
              <TableHead className="font-semibold text-slate-600 h-12">Phiên bản</TableHead>
          ...
          <TableBody>
            {curriculums.map((item) => (
              ...
              >
                ...
                <TableCell className="text-slate-600 font-medium max-w-[200px] truncate">
                  {item.description || "-"}
                </TableCell>
                <TableCell>
                  {item.isActive ? (
                  ...
                  {(canUpdate || canDelete) && (
                    <DropdownMenu>
                      <DropdownMenuTrigger
                        ...
                        aria-label="Tùy chọn giáo trình"
                      >
                          ...
                          <DropdownMenuItem
                            onClick={async () => {
                              const ok = await confirm({
                                ...
                                description: `Bạn có chắc chắn muốn xóa giáo trình ${item.name}? Hành động này không thể hoàn tác.`,
                                confirmLabel: "Xóa",
                                cancelLabel: "Hủy",
                                variant: "destructive",
                              });
